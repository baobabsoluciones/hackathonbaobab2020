...
def get_model():
    ...
    model.vResources = Var(
        model.sRResources, model.sJobs, model.sPeriods, domain=NonNegativeReals
    )
    ...
    def c1a_start_consistency(model, iJob):
        return model.vStart[iJob] == sum(
            model.v01Start[iJob, iPeriod] * int(iPeriod) for iPeriod in model.sPeriods
        )

    def c1b_end_consistency(model, iJob):
        return model.vEnd[iJob] == sum(
            model.v01End[iJob, iPeriod] * int(iPeriod) for iPeriod in model.sPeriods
        )
    ...
    def c3_amount_starts(model, iJob):
        return (
            sum(model.v01Start[iJob, iPeriod] for iPeriod in model.sPeriods) == 1
        )  # - model.vSlack[iJob]
    ...
    def c5_durations(model, iJob):
        return model.vEnd[iJob] + 1 >= model.vStart[iJob] + sum(
            model.pDuration[iJob, iMode] * model.v01Mode[iJob, iMode]
    ...
    def c7_resource_allocation(model, iJob, iMode, iPeriod, iResource):
        ...
        return (
            model.vResources[iResource, iJob, iPeriod]
    ...
    def c8_max_r_resources(model, iResource, iPeriod):
        return (
            sum(model.vResources[iResource, iJob, iPeriod] for iJob in model.sJobs)
    ...
    def c11_continue_work(model, iJob, iPeriod):
        if iPeriod < max(model.sPeriods):
            return (
                model.v01Work[iJob, iPeriod + 1]
                >= model.v01Work[iJob, iPeriod] - model.v01End[iJob, iPeriod]
            )
        else:
    ...
    model.c2_start_work_relation = Constraint(
        model.sJobs, model.sPeriods, rule=c2_start_work_relation
    )
    ...
    model.c7_resource_allocation = Constraint(
        model.sJobsModes, model.sPeriods, model.sRResources, rule=c7_resource_allocation
    )
    model.c8_max_r_resources = Constraint(
        model.sRResources, model.sPeriods, rule=c8_max_r_resources
    )
    ...
    model.c11_continue_work = Constraint(
        model.sJobs, model.sPeriods, rule=c11_continue_work
    )
