from hackathonbaobab2020.solver.milp_LP_HL.model import get_model
...
from hackathonbaobab2020.solver.milp_LP_HL.configuration import (
    SOLVER_PARAMETERS,
    MAX_PERIOD,
    MIN_ITERATION_TIME,
)
from hackathonbaobab2020.solver.milp_LP_HL.project_utils import (
    reverse_dict,
    get_status_value,
)
from hackathonbaobab2020.solver.milp_LP_HL.generic_iterator import BaseIterator
from hackathonbaobab2020.solver.milp_LP_HL.function_utils import Chrono
from hackathonbaobab2020.solver.milp_LP_HL.resources_subproblem import (
    solve_resource_subproblem,
)
...
class Iterator1(Experiment):
        ...
        if "timeLimit" in options:
            if "SOLVER_PARAMETERS" in options:
                ...
                options["SOLVER_PARAMETERS"] = {
                    "sec": max(int(options["timeLimit"] / 10), MIN_ITERATION_TIME)
                }
        else:
    ...
    def get_modes_order(self, current_resources=None):
        ...
        if current_resources is not None:
            resources_left = {
                r: (pMaxResources[r] - current_resources[r]) for r in sNResources
            }
        else:
        ...
        modes_n_resources = {
            (j, m): sum(
                (pResourcesUsed[(j, r, m)] / resources_left[r])
                if resources_left[r] > 0
                else 1000
                for r in sNResources
                if (j, r, m) in pResourcesUsed
            )
            for (j, m) in sJobsModes
        }

        jobs_modes = {j: [m for m in sModes if (j, m) in sJobsModes] for j in sJobs}

        modes_order = {
            j: sorted(jobs_modes[j], key=lambda m: modes_n_resources[j, m])
            for j in sJobs
        }
    ...
    def get_initial_solution(self, step, modes_steps):
        ...
        while k < max_jobs:
            ...
            max_period = int(
                ...
                + makespan
                + sum(
                    self.input_data["max_duration"][sJobs[j]] for j in range(k, max_j)
                )
            )
            ...
            free_indices["sJobsModes"] = [
                (j, m) for (j, m) in first_modes if j in free_indices["sJobs"]
            ]
            expected_resources = {
                r: sum(
                    self.input_data["pResourcesUsed"][j, r, m]
                    for j, m in free_indices["sJobsModes"]
                )
            ...
            free_jobs = [
                (i, j)
                for (i, j) in product(
                    free_indices["sJobs"] + fixed_indices["sJobs"],
                    free_indices["sJobs"] + fixed_indices["sJobs"],
                )
                if i != j
            ]
            activate_constraint(
                self.iterator.get_constraint("c10_precedence"), free_jobs
            )

            self.iteration[i] = self.iterator.iterate(
                free_indices,
                fixed_indices,
                excluded_constraints=["c10_precedence", "c9_max_n_resources"],
            )
            log.debug(
                "Iteration {} status: {} obj: {} ".format(
                    i, self.iteration[i][0], self.iteration[i][1]
                )
            )
            ...
            resources_used = {
                r: sum(
                    v01Mode[j, m] * self.input_data["pResourcesUsed"][j, r, m]
        ...
        if is_feasible(status):
            ...
            makespan = pyo.value(self.iterator.get_variable("vMakespan"))
            self.iterator.instance.sPeriods.values = [
                i for i in range(int(makespan + 1))
            ]
            self.input_data["sPeriods"][None] = [i for i in range(int(makespan + 1))]
    ...
    def solve_with_free_jobs_modes(self, free_jobs):
        ...
        free_indices = {
            "sJobs": sJobs,
            "sPeriods": sPeriods,
            "sJobsModes": free_jobs_modes,
            "sResources": self.input_data["sResources"][None],
        }
        fixed_indices = {
            "sJobs": [],
            "sPeriods": [],
            "sJobsModes": fixed_jobs_modes,
            "sResources": [],
        }
