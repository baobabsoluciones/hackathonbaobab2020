...
class Loop_solver(Experiment):
    ...
    def get_input_data(self):
        ...
        # self.input_data['max_duration'] = max_duration # delete
        # self.input_data["sRResources"] = {None: [r for r in resources if "R" in r]} # delete
        # self.input_data["sNResources"] = {None: [r for r in resources if "N" in r]} # delete
        # self.input_data["sJobsPrecedence"] = {None: jobs_precedence} # delete
        # self.input_data["sJobsModes"] = {None: [i for i in jobs_durations.keys()]} # delete
        # self.input_data["pWeightMakespan"] = {None: 1} # delete
        # self.input_data["pWeightResources"] = {None: 10} # delete
        ...
        for j in jobs:
            for m in modes:
                if (j, m) not in jobs_durations:
                    jobs_durations[(j, m)] = 0
                for r in resources:
                    if (j, m, r) not in resources_needs:
                        resources_needs[(j, m, r)] = 0
        ...
        # self.input_data["sSlots"] = {None: set(map(str,range(1, 40)))}
        ...
        # self.input_data['pNumberSlots'] = {None: 40}
